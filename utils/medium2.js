...
const compileFrontMatter = (story) => {
  let publishDate = new Date(story.date).toJSON();
  let author = {name: story.author};
...
const getContentAfterFirstFigure = (content) => {
  if ( content && content.indexOf('<figure>') === 0 && content.indexOf('</figure>') > 0 ){
...
const scrapeMedium = async () => {
  ...
  let previousPostMap = JSON.parse(previousPostsBody);
  let endpoint = 'https://api.rss2json.com/v1/api.json?rss_url=https%3A%2F%2Fmedium.com%2Ffeed%2Fcreme-de-la-crm';
  ...
  if ( data.items && data.items.length > 0){
    ...
    for ( let i=0; i<data.items.length; i++ ){
      ...
      if ( !previousPostMap[mediumLink] ){
        ...
        toDownload.push( utils.saveFile(story.localFileName+'.md',story.mdContent));
        if ( story.mediumImagesToDownload ){
          for ( let i=0; i<story.mediumImagesToDownload.length; i++ ){
            ...
            const ourImageUrl = '/img/'+utils.cleanFileName(utils.substringafterlast(mediumImageUrl,'/'));
...
const convertRssItem = (item) => {
  ...
  const imgRex = /https:\/\/cdn-images-1.medium.com\/([^"]*.")/gmi;
  const rawImages = story.rssContent.match(imgRex);
  story.mediumImagesToDownload = [];
  if ( rawImages && rawImages.length > 0 ){
    for ( let i=0; i<rawImages.length; i++ ){
      let src = rawImages[i].replace(/"/,'').replace("'",'');
      story.mediumImagesToDownload.push(src);
    }
  }
  story.thumbnailURL = utils.substringafter(utils.cleanFileName(story.mediumImagesToDownload[0]),'.com/');
  story.thumbnailURL = story.thumbnailURL.substring(0,story.thumbnailURL.length-1);
  ...
  if ( story.mdContent.indexOf('<p>') >= 0 && story.mdContent.indexOf('<p>') < 5 ){
  ...
  if ( figureTags && figureTags.length > 0 ){
    for ( let i=0; i<figureTags.length; i++ ){
      let oldTag = figureTags[i];
      let src = utils.cleanFileName(utils.substringbetween( oldTag, 'src="', '"'));
      src = src.replace('https://cdn_images_1.medium.com','/img');
      let newTag = '{{< image src="'+src+'" title="Image" >}}';
  ...
  if ( h3Tags && h3Tags.length > 0 ){
    for ( let i=0; i<h3Tags.length; i++ ){
      ...
      story.mdContent = story.mdContent.replace(h3Tags[i],'\n## '+newTag+'\n');
    }
  }

  const blockquoteRegex = /<blockquote(.*)<\/blockquote>/gmi;
  ...
  if ( blockquoteTags && blockquoteTags.length > 0 ){
    for ( let i=0; i<blockquoteTags.length; i++ ){
     let newTag = utils.substringbetween(blockquoteTags[i],'<blockquote>','<\/blockquote');
      newTag = newTag.replace(/<br>/gmi,'  \n>')
      story.mdContent = story.mdContent.replace(blockquoteTags[i],'\n> '+newTag+'\n');
    }
  }

  story.mdContent = compileFrontMatter(story) + '\n\n\n'+ story.mdContent;
  if ( story.mdContent.indexOf('<img src="https://medium.com/_/stat') > 0 ){
    story.mdContent = utils.substringbefore(story.mdContent,'<img src="https://medium.com/_/stat');
  }
  return story;
}
