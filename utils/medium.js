...
const getContentAfterFirstFigure = (content) => {
  if (content && content.indexOf('<figure>') >= 0 && content.indexOf('</figure>') > content.indexOf('<figure>')) {
...
const convertRssItem = (item) => {
  ...
  story.thumbnailURL = story.thumbnailURL.substring(0, story.thumbnailURL.length)
  ...
  } else if (story.mdContent.indexOf('<p>') >= 0 && story.mdContent.indexOf('<p>') < 5) {
  ...
  if (story.mdContent.indexOf(story.thumbnailURL) > 0) {
    ...
    story.mdContent = utils.substringafter(story.mdContent, 'title="Image" >}}')
  }
